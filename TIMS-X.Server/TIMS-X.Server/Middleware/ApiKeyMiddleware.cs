...
namespace TIMS_X.Server.Middleware
{
    ...
    public class ApiKeyMiddleware
    {
        ...
        private VendorQuery _vendorQuery;
        ...
        public void LogRequest(string requestPath, bool accessGranted, string vendorName, string officeCode, string callerIp)
        {
            Log.Information($"{(accessGranted ? "Accepted" : "Denied")} request to {requestPath}. Vendor: {vendorName}, Customer: {officeCode}, Caller Ip: {callerIp}");
        }
		...
		/// <returns></returns>
		public async Task Invoke(HttpContext context, VendorQuery vendorQuery)
        {
            ...
            {
                keyRequired = true;
                if(context.Request.Path.Value.Contains("SqlConnectionDetails"))
                {
                    systemOnly = true;
                }
        ...
        public async Task Validate(HttpContext context, bool systemOnly, string apiKey, string officeCode)
        {
            ...
            var vendor = await _vendorQuery.GetVendorAsyncQuery(apiKey);
