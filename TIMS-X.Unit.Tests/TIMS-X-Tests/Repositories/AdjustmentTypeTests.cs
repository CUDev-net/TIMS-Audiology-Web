...
public class AdjustmentTypeTests : TestObjectBase<AdjustmentType>
{
    [Theory]
    ...
    {
        // Arrange
        var adjustmentTypes = new List<AdjustmentType>
        {
            new() { Name = "New" }
        };
        var adjustTypeUoW = new Mock<IAdjustmentTypeUnitOfWork>();
        adjustTypeUoW
            .Setup(x => x.GetAdjustmentTypes(It.IsAny<Expression<Func<AdjustmentType, bool>>>(), null,
                null)).Returns(Task.FromResult(adjustmentTypes));
        var validator = new AdjustmentTypeValidator(adjustTypeUoW.Object);

        // Act
        List<ValidationResult>? validationResults = operationType switch
        {
            OperationType.AddNew => await validator.AddNew(new AdjustmentType { Name = newName }),
            OperationType.Update => await validator.Update(new AdjustmentType { Name = newName }),
            _ => null
        };
    ...
    public async void ValidateAppointmentTypeIsUnique_ReturnsNoError(OperationType operationType, string newName)
    {
        ...
        adjustTypeUoW
            .Setup(x => x.GetAdjustmentTypes(It.IsAny<Expression<Func<AdjustmentType, bool>>>(), null,
                null)).Returns(Task.FromResult(new List<AdjustmentType>()));
        var validator = new AdjustmentTypeValidator(adjustTypeUoW.Object);
