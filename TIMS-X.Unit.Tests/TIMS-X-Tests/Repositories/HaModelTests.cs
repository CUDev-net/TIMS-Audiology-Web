...
public class HaModelTests : TestObjectBase<HaModel>
{
    [Theory]
    ...
    {
        // Arrange
        var haModels = new List<HaModel>
        {
            new() { Name = "New" }
        };
        var haModelUoW = new Mock<IHaModelUnitOfWork>();
        haModelUoW
            .Setup(x => x.GetHaModels(It.IsAny<Expression<Func<HaModel, bool>>>(),
                null,
                null)).Returns(Task.FromResult(haModels));
        var validator = new HaModelValidator(haModelUoW.Object);

        // Act
        List<ValidationResult>? validationResults = operationType switch
        {
            OperationType.AddNew => await validator.AddNew(new HaModel { Name = newName }),
            OperationType.Update => await validator.Update(new HaModel { Name = newName }),
            _ => null
        };
    ...
    public async void ValidateHaModelIsUnique_ReturnsNoError(OperationType operationType,
        string newName)
    {
        // Arrange
        var haModelUoW = new Mock<IHaModelUnitOfWork>();
        haModelUoW
            .Setup(x => x.GetHaModels(It.IsAny<Expression<Func<HaModel, bool>>>(),
                null,
                null)).Returns(Task.FromResult(new List<HaModel>()));
        var validator = new HaModelValidator(haModelUoW.Object);
