...
public class HaStatusTests : TestObjectBase<HaStatus>
{
    [Theory]
    ...
    {
        // Arrange
        var haStatuses = new List<HaStatus>
        {
            new() { Name = "New" }
        };
        var haStatusUoW = new Mock<IHaStatusUnitOfWork>();
        haStatusUoW
            .Setup(x => x.GetHaStatuses(It.IsAny<Expression<Func<HaStatus, bool>>>(), null,
                null)).Returns(Task.FromResult(haStatuses));
        var validator = new HaStatusValidator(haStatusUoW.Object);

        // Act
        List<ValidationResult>? validationResults = operationType switch
        {
            OperationType.AddNew => await validator.AddNew(new HaStatus { Name = newName }),
            OperationType.Update => await validator.Update(new HaStatus { Name = newName }),
            _ => null
        };
    ...
    public async void ValidateHaStatusIsUnique_ReturnsNoError(OperationType operationType, string newName)
    {
        // Arrange
        var haStatusUoW = new Mock<IHaStatusUnitOfWork>();
        haStatusUoW
            .Setup(x => x.GetHaStatuses(It.IsAny<Expression<Func<HaStatus, bool>>>(), null,
                null)).Returns(Task.FromResult(new List<HaStatus>()));
        var validator = new HaStatusValidator(haStatusUoW.Object);
