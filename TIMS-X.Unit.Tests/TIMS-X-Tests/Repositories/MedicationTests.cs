...
public class MedicationTests : TestObjectBase<Medication>
{
    [Theory]
    ...
    public async void ValidateAlreadyExists_ReturnsError(OperationType operationType, string newName)
    {
        ...
        var medicationUoW = new Mock<IMedicationUnitOfWork>();
        medicationUoW
            .Setup(x => x.GetMedications(It.IsAny<Expression<Func<Medication, bool>>>(), null,
                null)).Returns(Task.FromResult(medications));
        var validator = new MedicationValidator(medicationUoW.Object);

        // Act
        List<ValidationResult>? validationResults = operationType switch
        {
            OperationType.AddNew => await validator.AddNew(new Medication { Name = newName }),
            OperationType.Update => await validator.Update(new Medication { Name = newName }),
            _ => null
        };
    ...
    public async void ValidateMedicationIsUnique_ReturnsNoError(OperationType operationType, string newName)
    {
        // Arrange
        var medicationUoW = new Mock<IMedicationUnitOfWork>();
        medicationUoW
            .Setup(x => x.GetMedications(It.IsAny<Expression<Func<Medication, bool>>>(), null,
                null)).Returns(Task.FromResult(new List<Medication>()));
        var validator = new MedicationValidator(medicationUoW.Object);
