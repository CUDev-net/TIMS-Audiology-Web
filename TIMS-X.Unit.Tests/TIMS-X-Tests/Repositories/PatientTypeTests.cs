...
public class PatientTypeTests : TestObjectBase<PatientType>
{
    [Theory]
    ...
    public async void ValidateAlreadyExists_ReturnsError(OperationType operationType, string newName)
    {
        ...
        var patientTypeUoW = new Mock<IPatientTypeUnitOfWork>();
        patientTypeUoW
            .Setup(x => x.GetPatientTypes(It.IsAny<Expression<Func<PatientType, bool>>>(), null,
                null)).Returns(Task.FromResult(patientTypes));
        var validator = new PatientTypeValidator(patientTypeUoW.Object);

        // Act
        List<ValidationResult>? validationResults = operationType switch
        {
            OperationType.AddNew => await validator.AddNew(new PatientType { Name = newName }),
            OperationType.Update => await validator.Update(new PatientType { Name = newName }),
            _ => null
        };
    ...
    public async void ValidatePatientTypeIsUnique_ReturnsNoError(OperationType operationType, string newName)
    {
        // Arrange
        var patientTypeUoW = new Mock<IPatientTypeUnitOfWork>();
        patientTypeUoW
            .Setup(x => x.GetPatientTypes(It.IsAny<Expression<Func<PatientType, bool>>>(), null,
                null)).Returns(Task.FromResult(new List<PatientType>()));
        var validator = new PatientTypeValidator(patientTypeUoW.Object);
