...
public class SalutationTests : TestObjectBase<Salutation>
{
    [Theory]
    ...
    public async void ValidateAlreadyExists_ReturnsError(OperationType operationType, string newName)
    {
        ...
        var salutationUoW = new Mock<ISalutationUnitOfWork>();
        salutationUoW
            .Setup(x => x.GetSalutations(It.IsAny<Expression<Func<Salutation, bool>>>(), null,
                null)).Returns(Task.FromResult(salutations));
        var validator = new SalutationValidator(salutationUoW.Object);

        // Act
        List<ValidationResult>? validationResults = operationType switch
        {
            OperationType.AddNew => await validator.AddNew(new Salutation { Name = newName }),
            OperationType.Update => await validator.Update(new Salutation { Name = newName }),
            _ => null
        };
    ...
    public async void ValidateSalutationIsUnique_ReturnsNoError(OperationType operationType, string newName)
    {
        // Arrange
        var salutationUoW = new Mock<ISalutationUnitOfWork>();
        salutationUoW
            .Setup(x => x.GetSalutations(It.IsAny<Expression<Func<Salutation, bool>>>(), null,
                null)).Returns(Task.FromResult(new List<Salutation>()));
        var validator = new SalutationValidator(salutationUoW.Object);
