...
public class SexTests : TestObjectBase<Sex>
{
    [Theory]
    ...
    public async void ValidateAlreadyExists_ReturnsError(OperationType operationType, string newName)
    {
        ...
        var sexUoW = new Mock<ISexUnitOfWork>();
        sexUoW
            .Setup(x => x.GetSexes(It.IsAny<Expression<Func<Sex, bool>>>(), null,
                null)).Returns(Task.FromResult(sexes));
        var validator = new SexValidator(sexUoW.Object);

        // Act
        List<ValidationResult>? validationResults = operationType switch
        {
            OperationType.AddNew => await validator.AddNew(new Sex { Name = newName }),
            OperationType.Update => await validator.Update(new Sex { Name = newName }),
            _ => null
        };
    ...
    public async void ValidateSexIsUnique_ReturnsNoError(OperationType operationType, string newName)
    {
        // Arrange
        var sexUoW = new Mock<ISexUnitOfWork>();
        sexUoW
            .Setup(x => x.GetSexes(It.IsAny<Expression<Func<Sex, bool>>>(), null,
                null)).Returns(Task.FromResult(new List<Sex>()));
        var validator = new SexValidator(sexUoW.Object);
