...
public class TaxAgencyTests : TestObjectBase<TaxAgency>
{
    [Theory]
    ...
    {
        // Arrange
        var taxAgencies = new List<TaxAgency>
        {
            new() { Name = "New" }
        };
        var taxAgencyUoW = new Mock<ITaxAgencyUnitOfWork>();
        taxAgencyUoW
            .Setup(x => x.GetTaxAgencies(It.IsAny<Expression<Func<TaxAgency, bool>>>(), null,
                null)).Returns(Task.FromResult(taxAgencies));
        var validator = new TaxAgencyValidator(taxAgencyUoW.Object);

        // Act
        List<ValidationResult>? validationResults = operationType switch
        {
            OperationType.AddNew => await validator.AddNew(new TaxAgency { Name = newName }),
            OperationType.Update => await validator.Update(new TaxAgency { Name = newName }),
            _ => null
        };
    ...
    public async void ValidateTaxAgencyIsUnique_ReturnsNoError(OperationType operationType, string newName)
    {
        // Arrange
        var taxAgencyUoW = new Mock<ITaxAgencyUnitOfWork>();
        taxAgencyUoW
            .Setup(x => x.GetTaxAgencies(It.IsAny<Expression<Func<TaxAgency, bool>>>(), null,
                null)).Returns(Task.FromResult(new List<TaxAgency>()));
        var validator = new TaxAgencyValidator(taxAgencyUoW.Object);
